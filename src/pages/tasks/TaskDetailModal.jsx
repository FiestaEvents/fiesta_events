...
import {
  Trash2, Edit, Archive, RotateCcw, Clock, AlertCircle, User, Calendar, Tag, ArrowRight, CheckCircle, AlertTriangle, Users, FileText
} from "lucide-react";
...
const TaskDetailModal = ({ isOpen, onClose, task, onEdit, refreshData, showArchived = false }) => {
  ...
  const getStatusVariant = (s) => {
    const map = { 
      completed: "success", active: "info", pending: "warning", todo: "primary", in_progress: "purple", blocked: "danger", cancelled: "secondary" 
    };
  ...
  const handleArchive = () => handleAction(
    () => taskService.archive(task._id),
    { 
  ...
  const handleRestore = () => handleAction(
    () => taskService.unarchive(task._id),
    { 
  ...
  const InfoRow = ({ icon: Icon, label, value, color="blue" }) => (
    <div className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors border border-transparent hover:border-gray-100">
      <div className={`p-2 rounded-full bg-${color}-50 text-${color}-600 dark:bg-${color}-900/20 dark:text-${color}-400`}>
        <Icon size={16} />
      </div>
      <div>
  ...
  return (
    <>
      <Modal isOpen={isOpen} onClose={onClose} title={t("taskDetailModal.title")} size="md">
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-2">
            <InfoRow icon={Calendar} label={t('taskDetailModal.dueDate')} value={formatDate(task.dueDate)} color={isOverdue(task.dueDate, task.status) ? "red" : "blue"} />
            <InfoRow icon={User} label={t('taskDetailModal.assignedTo')} value={task.assignedTo?.name || t('tasks.unassigned')} color="purple" />
            {task.estimatedHours && <InfoRow icon={Clock} label={t('taskDetailModal.estimatedHours')} value={`${task.estimatedHours}h`} color="orange" />}
          </div>
          ...
          {task.tags && task.tags.length > 0 && (
            <div className="flex flex-wrap gap-2 pt-2">
              {task.tags.map((tag, idx) => (
                <Badge key={idx} variant="secondary" icon={<Tag size={10}/>} className="text-xs">
                  {tag}
                </Badge>
              ))}
            </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between gap-3 pt-4 border-t border-gray-100 dark:border-gray-700">
            <div className="flex gap-2 w-full sm:w-auto">
              {!showArchived ? (
                ...
                <>
                  <Button variant="outline" icon={RotateCcw} onClick={handleRestore} disabled={isProcessing} size="sm">
                    {t('common.restore')}
                  </Button>
                  <Button variant="danger" icon={Trash2} onClick={() => setShowDeleteConfirm(true)} disabled={isProcessing} size="sm">
                    {t('common.delete')}
                  </Button>
                </>
              )}
      ...
      <Modal isOpen={showDeleteConfirm} onClose={() => setShowDeleteConfirm(false)} title={t("common.confirmDelete")} size="sm">
        <div className="p-6 text-center">
          ...
          <div className="flex justify-center gap-3">
            ...
            <Button variant="danger" loading={isProcessing} onClick={handleDelete}>{t("common.delete")}</Button>
          </div>
        </div>
      </Modal>
    </>
