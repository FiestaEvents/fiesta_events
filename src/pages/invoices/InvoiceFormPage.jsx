    ...
    <div className="group p-5 rounded-xl border border-orange-200 dark:border-orange-800/50 bg-gradient-to-br from-orange-50 to-white dark:from-gray-800 dark:to-gray-800/50 relative overflow-hidden shadow-sm">
      ...
      <div className="flex items-start gap-4 relative z-10">
        ...
        {onClear && (
          <button
            onClick={onClear}
            className="p-2 rounded-lg hover:bg-white dark:hover:bg-gray-700 text-gray-400 hover:text-red-500 shadow-sm transition-all"
          >
            <X size={18} />
          </button>
        )}
...
const InvoiceFormPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!isEditMode) {
          const bankDetails =
            stRes.data?.paymentTerms?.bankDetails ||
            stRes?.paymentTerms?.bankDetails;
  ...
  useEffect(() => {
    if (!isEditMode && !fetchLoading) {
      const hasContent =
        formData.client || formData.partner || formData.items[0]?.description;
      if (hasContent) {
        const draftObj = {
          ...formData,
          invoiceType,
  ...
  useEffect(() => {
    ...
    formData.items.forEach(
      (i) => (sub += (Number(i.quantity) || 0) * (Number(i.rate) || 0))
    );
  ...
  const activeRecipient = recipients.find(
    (r) =>
      r._id === (invoiceType === "client" ? formData.client : formData.partner)
  );

  const filteredEvents = useMemo(() => {
    const recipientId =
      invoiceType === "client" ? formData.client : formData.partner;
    ...
    return events.filter((evt) => {
      ...
      return evt.partners?.some(
        (p) => (p.partner?._id || p.partner) === recipientId
      );
    });
  ...
  const handleItemChange = (idx, field, val) => {
    ...
    items[idx].amount =
      (Number(items[idx].quantity) || 0) * (Number(items[idx].rate) || 0);
