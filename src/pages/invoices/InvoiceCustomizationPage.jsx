...
import {
  ...
  RotateCcw, Table as TableIcon, Grid, Move,
  ZoomIn, ZoomOut, Layers, DollarSign, ArrowLeft, Eye
} from "lucide-react";
...
const ColorPicker = ({ label, value, onChange }) => {
  ...
  return (
    <div className="relative">
      ...
      {isOpen && (
        <div ref={popover} className="absolute top-full left-0 mt-2 z-50 bg-white dark:bg-gray-800 p-3 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700">
...
// HELPER: RANGE SLIDER
// ============================================
const RangeSlider = ({ label, icon: Icon, value, min, max, step, suffix, onChange }) => (
...
const ModernSwitch = ({ label, checked, onChange }) => (
  <label className="flex items-center justify-between p-3 border border-gray-200 dark:border-gray-700 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
    ...
    <div className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${checked ? 'bg-orange-500' : 'bg-gray-300 dark:bg-gray-600'}`}>
...
const InvoiceSettingsPage = () => {
  ...
  const previewData = {
    ...
    recipient: { name: "Client Exemple", email: "client@example.com", phone: "+216 20 000 000" },
    items: [
        { description: "Location Salle de Conférence", quantity: 1, rate: 1500, amount: 1500 },
        { description: "Pause Café (Matin)", quantity: 50, rate: 12, amount: 600 },
  ...
  const fetchSettings = useCallback(async () => {
    try {
      ...
      showError(t('invoiceCustomization.toasts.loadError', "Failed to load settings."));
    } finally {
      setLoading(false);
    }
  }, [showError, t]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showSuccess(t('invoiceCustomization.toasts.saveSuccess', "Design enregistré !"));
    } catch (error) {
      showError(t('invoiceCustomization.toasts.saveError', "Erreur lors de l'enregistrement."));
    } finally {
      setSaving(false);
    }
  };
  ...
  const TABS = [
    { id: 'branding', label: t('invoiceCustomization.tabs.brand', 'Branding'), icon: Palette },
    { id: 'layout', label: t('invoiceCustomization.tabs.layout', 'Mise en page'), icon: Layers },
    { id: 'table', label: t('invoiceCustomization.tabs.table', 'Tableau'), icon: TableIcon },
    { id: 'style', label: t('invoiceCustomization.tabs.style', 'Style'), icon: Layout },
    { id: 'text', label: t('invoiceCustomization.tabs.content', 'Textes'), icon: Type },
  ];
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-gray-900 overflow-hidden font-sans" dir={isRTL ? "rtl" : "ltr"}>
      ...
      <div className={`flex-1 flex flex-col h-full overflow-hidden border-r border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 z-10 shadow-xl max-w-3xl ${isRTL ? "border-l border-r-0" : ""}`}>
        ...
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center shrink-0 bg-white dark:bg-gray-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-gray-900 dark:text-white text-lg">{t('invoiceCustomization.title', 'Design Facture')}</h1>
              <p className="text-xs text-gray-500">{t('invoiceCustomization.subtitle', 'Configuration')}</p>
            </div>
          </div>
          <Button variant="primary" icon={Save} onClick={handleSave} loading={saving}>{t('common.save', 'Save')}</Button>
        </div>

        <div className="flex flex-1 overflow-hidden">
            {/* Tabs Sidebar */}
            <div className={`w-64 overflow-y-auto p-4 bg-gray-50/50 dark:bg-gray-800/50 shrink-0 ${isRTL ? "border-l" : "border-r"} border-gray-100 dark:border-gray-700`}>
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="relative h-32 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-900 hover:bg-orange-50 dark:hover:bg-orange-900/10 transition-all flex flex-col items-center justify-center overflow-hidden group">
                                ...
                                {settings.branding.logo.url ? (
                                    <img src={settings.branding.logo.url} className="h-full object-contain p-2" alt="Logo" />
                                ) : (
                                    <div className="flex flex-col items-center text-gray-400 group-hover:text-orange-500">
                                        <ImageIcon size={24} />
                                        <span className="text-xs font-medium mt-2">Uploader un logo</span>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="p-5 bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <ColorPicker label="Couleur Primaire" value={settings.branding.colors.primary} onChange={v => updateSetting('branding.colors.primary', v)} />
                                <ColorPicker label="Couleur Secondaire" value={settings.branding.colors.secondary} onChange={v => updateSetting('branding.colors.secondary', v)} />
                                <ColorPicker label="Couleur Texte" value={settings.branding.colors.text} onChange={v => updateSetting('branding.colors.text', v)} />
                            </div>
                        </div>
                    </div>
                )}
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
                        ...
                        <div className="space-y-3">
                            {settings.layout.sections.sort((a, b) => a.order - b.order).map((section, index) => (
                                <div key={section.id} className="flex items-center gap-3 p-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm">
                                    <input 
                                        type="checkbox" 
                                    ...
                                    />
                                    <span className="flex-1 text-sm font-medium text-gray-700 dark:text-white">{section.label}</span>
                ...
                {activeTab === 'table' && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
                        ...
                        <div className="p-5 bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 space-y-4">
                            <ColorPicker label="Fond En-tête" value={settings.table.headerColor || settings.branding.colors.primary} onChange={v => updateSetting('table.headerColor', v)} />
                            <div className="grid grid-cols-2 gap-4 pt-2">
                                <ModernSwitch label="Lignes alternées" checked={settings.table.striped} onChange={e => updateSetting('table.striped', e.target.checked)} />
                                <ModernSwitch label="Bords arrondis" checked={settings.table.rounded} onChange={e => updateSetting('table.rounded', e.target.checked)} />
                            </div>
                        </div>

                        <div className="space-y-3">
                             <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Colonnes Visibles</label>
                             {Object.entries(settings.table.columns).map(([key, val]) => (
                                <ModernSwitch key={key} label={key.charAt(0).toUpperCase() + key.slice(1)} checked={val} onChange={e => updateSetting(`table.columns.${key}`, e.target.checked)} />
                             ))}
                        </div>
                    </div>
                )}
                ...
                {activeTab === 'style' && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
                        ...
                        <div className="p-5 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm space-y-6">
                            <RangeSlider label="Taille Police" icon={Type} value={settings.branding.fonts.size} min={8} max={14} step={1} suffix="px" onChange={e => updateSetting('branding.fonts.size', parseInt(e.target.value))} />
                            <div className="h-px bg-gray-100 dark:bg-gray-700"></div>
                            <RangeSlider label="Arrondi" icon={Grid} value={settings.layout.borderRadius} min={0} max={24} step={2} suffix="px" onChange={e => updateSetting('layout.borderRadius', parseInt(e.target.value))} />
                        </div>
                    </div>
                )}
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            {['invoiceTitle', 'from', 'to', 'item', 'total'].map(label => (
                                <Input key={label} label={label.toUpperCase()} value={settings.labels[label]} onChange={e => updateSetting(`labels.${label}`, e.target.value)} />
                            ))}
                        </div>
                        <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
                            <label className="block text-xs font-bold text-gray-500 uppercase mb-2 flex items-center gap-2"><DollarSign size={14} /> Instructions de Paiement</label>
                            <textarea 
                                rows={4} 
                                value={settings.paymentTerms.bankDetails} 
                                onChange={e => updateSetting('paymentTerms.bankDetails', e.target.value)} 
      ...
      <div className={`hidden lg:flex flex-1 bg-gray-900 flex-col overflow-hidden ${isRTL ? "border-r" : "border-l"} border-gray-700`}>
        <div className="h-14 flex items-center justify-between px-6 border-b border-gray-700 bg-gray-900 shrink-0">
          ...
          <div className="text-gray-500 text-xs flex items-center gap-2"><Eye size={14} /> Live Preview</div>
        </div>
