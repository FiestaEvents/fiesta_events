...
const Step5Review = () => {
  ...
  const ReviewRow = ({ label, value, highlight = false }) => (
    <div className={`flex justify-between items-center py-2.5 sm:py-3 border-b border-gray-100 dark:border-gray-700 last:border-0 ${highlight ? 'bg-gray-50 dark:bg-gray-700/30 -mx-3 sm:-mx-4 px-3 sm:px-4 rounded-lg mt-2' : ''}`}>
      <span className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">{label}</span>
      <span className={`text-xs sm:text-sm font-medium text-right ${highlight ? 'text-gray-900 dark:text-white text-sm sm:text-base' : 'text-gray-900 dark:text-white'}`}>
        {value || "-"}
      </span>
    </div>
  );
    ...
    <div className="space-y-4 sm:space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 h-full">
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-2 py-2.5 sm:py-3 border-b border-gray-100 dark:border-gray-700">
              <User className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-400 shrink-0" />
              <span className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 flex-1">{t('eventForm.step2.client')}</span>
              <span className="text-xs sm:text-sm font-semibold text-blue-600 dark:text-blue-400 text-right">{selectedClient?.name}</span>
            </div>

            <div className="flex items-center gap-2 py-2.5 sm:py-3 border-b border-gray-100 dark:border-gray-700 last:border-0">
              <MapPin className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-400 shrink-0" />
              <span className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 flex-1">{t('eventForm.step3.venue')}</span>
              <span className="text-xs sm:text-sm font-semibold text-purple-600 dark:text-purple-400 text-right">{selectedSpace?.name}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 h-full flex flex-col">
          ...
          <div className="space-y-1 flex-1">
            ...
            {/* Total Highlight */}
            <div className="mt-3 sm:mt-4 p-3 sm:p-4 bg-orange-50 dark:bg-orange-900/10 rounded-xl border border-orange-100 dark:border-orange-900/30 flex justify-between items-center">
              <span className="font-bold text-orange-800 dark:text-orange-300 uppercase tracking-wider text-[10px] sm:text-xs">
                Total Estimated
              </span>
              <span className="text-xl sm:text-2xl font-bold text-orange-600 dark:text-orange-400">
                {calculations.totalPrice?.toFixed(2)} <span className="text-xs sm:text-sm font-medium">TND</span>
              </span>
            </div>
          </div>
          ...
          {!meta.isEditMode && (
            <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-gray-100 dark:border-gray-700">
              <div className="flex flex-col xs:flex-row xs:items-center justify-between gap-3 p-3 sm:p-4 bg-gray-50 dark:bg-gray-700/30 rounded-xl border border-gray-200 dark:border-gray-600 transition-colors hover:border-blue-300 dark:hover:border-blue-700">
                <div className="flex items-center gap-2 sm:gap-3">
                  <div className="p-1.5 sm:p-2 bg-white dark:bg-gray-800 rounded-full shadow-sm text-blue-500 shrink-0">
      ...
      <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
        ...
        <Textarea 
          ...
          className="bg-gray-50 dark:bg-gray-900/50 border-gray-200 dark:border-gray-700 resize-none focus:bg-white dark:focus:bg-gray-900 text-sm"
        />
      </div>
    </div>
  );
