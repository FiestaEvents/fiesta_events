...
const PaymentForm = ({ payment, onSuccess, onCancel }) => {
    ...
    if (formData.client) {
      if (events.length > 0) {
        const clientEvents = events.filter((e) => {
          const cId =
            e.clientId?._id || e.clientId || e.client?._id || e.client;
  ...
  const handleEventChange = (e) => {
    ...
    if (evtId) {
      ...
      const cId =
        evt?.clientId?._id || evt?.clientId || evt?.client?._id || evt?.client;
  ...
  const validateStep = (step) => {
    ...
    if (step === 3) {
      if (
        formData.paidDate &&
        formData.dueDate &&
        new Date(formData.paidDate) < new Date(formData.dueDate)
      ) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validateStep(4))
      return showError(t("payments.form.errors.fixAllErrors"));
    await submitData();
  };
        ...
        {steps.map((step, idx) => {
          ...
          return (
            ...
            >
              ...
              <span
                className={`text-xs font-medium mt-2 ${isActive ? "text-orange-600" : "text-gray-500"}`}
              >
                {step.title}
              </span>
              {idx < steps.length - 1 && (
            ...
            <div className="space-y-6 animate-in fade-in">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <Input
                  ...
                  onChange={(e) =>
                    handleChange("fees.processingFee", e.target.value)
                  }
                  placeholder="0.00"
                />
                <Input
                  ...
                  onChange={(e) =>
                    handleChange("fees.platformFee", e.target.value)
                  }
                  placeholder="0.00"
                />
                <Input
                  ...
                  onChange={(e) =>
                    handleChange("fees.otherFees", e.target.value)
                  }
                  placeholder="0.00"
                />
              </div>
