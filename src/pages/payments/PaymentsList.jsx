...
const PaymentsList = () => {
  ...
  const tableColumns = [
      ...
      render: (row) => (
        <Badge variant={getBadgeVariant(row.type, 'type')}>
          <div className="flex items-center gap-1">
            {row.type === PAYMENT_TYPES.INCOME ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />}
    ...
    },
    {
      ...
      width: "13%",
      render: (row) => (
        <div className={`font-bold ${row.type === PAYMENT_TYPES.INCOME ? "text-green-600" : "text-red-600"}`}>
          {row.type === PAYMENT_TYPES.INCOME ? "+" : "-"}{formatCurrency(row.amount)}
        </div>
      )
    },
    {
      ...
      accessor: "status",
      width: "10%",
      render: (row) => <Badge variant={getBadgeVariant(row.status, 'status')}>{t(`payments.statuses.${row.status}`) || "Unknown"}</Badge>
    },
    {
      ...
      render: (row) => (
        <div className="flex justify-center gap-2">
          <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); navigate(`/payments/${row._id}`); }} className="text-gray-500 hover:text-orange-600"><Eye className="h-4 w-4" /></Button>
          <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); openPaymentForm(row); }} className="text-blue-500 hover:text-blue-700"><Edit className="h-4 w-4" /></Button>
          {row.type === PAYMENT_TYPES.INCOME && [PAYMENT_STATUSES.COMPLETED, PAYMENT_STATUSES.PAID].includes(row.status?.toLowerCase()) && (
            <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); setSelectedPayment(row); setIsRefundModalOpen(true); }} className="text-yellow-500 hover:text-yellow-700"><RotateCcw className="h-4 w-4" /></Button>
          )}
          <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); handleDeletePayment(row._id, row.description); }} className="text-red-500 hover:text-red-700"><Trash2 className="h-4 w-4" /></Button>
        </div>
      )
    }
  ];
    ...
    return (
      <div className="mt-6 border-t border-gray-200 dark:border-gray-700 pt-4 flex flex-col sm:flex-row justify-between items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
          {totalPages > 1 && (
             <Pagination currentPage={filters.page} totalPages={totalPages} onPageChange={(page) => updateFilter('page', page)} pageSize={null} />
          )}
  ...
  return (
    <div className="space-y-6 p-6 bg-white dark:bg-[#1f2937] rounded-lg shadow-md min-h-[500px] flex flex-col">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 shrink-0">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400">
            {t('payments.subtitle')}
            {hasInitialLoad && !showEmptyState && <span className="ml-1">({totalCount})</span>}
          </p>
        </div>
        ...
        {!showEmptyState && (
          <div className="flex gap-2 w-full sm:w-auto">
            {totalCount > 0 && (
              <Button variant="outline" onClick={handleExportCSV} className="flex items-center gap-2 flex-1 sm:flex-none justify-center">
                <Download className="h-4 w-4" />{t('payments.exportPayments')}
              </Button>
            )}
            <Button variant="primary" onClick={() => openPaymentForm()} className="flex items-center gap-2 flex-1 sm:flex-none justify-center">
      ...
      {!showEmptyState && payments.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 shrink-0">
          {STAT_CONFIGS.map(config => {
            const Icon = config.icon;
            if (config.isStatusCard) {
              return (
                <div key={config.key} className={`p-4 rounded-lg border ${config.bgClass} ${config.borderClass}`}>
                  <div className="flex items-center justify-between mb-3">
                    <div className={`text-sm font-medium ${config.textClass}`}>{t(config.labelKey)}</div>
                    <div className={`p-2 rounded-lg ${config.iconBgClass}`}><Icon className={`w-5 h-5 ${config.iconClass}`} /></div>
                  </div>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="flex items-center gap-2 text-gray-600 dark:text-gray-400"><span className="w-2 h-2 bg-green-500 rounded-full" />{t('payments.statuses.completed')}</span>
                      <span className="font-bold dark:text-white">{stats.completedPayments}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="flex items-center gap-2 text-gray-600 dark:text-gray-400"><span className="w-2 h-2 bg-yellow-500 rounded-full" />{t('payments.statuses.pending')}</span>
            ...
            return (
              <div key={config.key} className={`p-4 rounded-lg border ${config.bgClass} ${config.borderClass}`}>
                ...
                <div className="space-y-1">
                  <div className={`text-2xl font-bold ${config.isDynamic && mainValue < 0 ? 'text-red-600' : config.valueClass}`}>
      ...
      {!showEmptyState && (
        <div className="p-4 bg-white dark:bg-gray-800 rounded-lg flex flex-col sm:flex-row gap-4 shrink-0">
            ...
            <div className="sm:w-40">
            <Select value={filters.status} onChange={(e) => updateFilter('status', e.target.value)} options={[{ value: "all", label: "All Status" }, { value: "pending", label: "Pending" }, { value: "completed", label: "Completed" }]} />
            </div>
            {hasActiveFilters && (
        ...
        {showNoResults && (
            <div className="flex flex-col items-center justify-center flex-1 py-12">
                ...
                <p className="text-gray-500 dark:text-gray-400 text-center max-w-sm mb-6">
                    {t('payments.filters.noResultsDesc', "Try adjusting your filters to find what you're looking for.")}
                </p>
            ...
            <div className="flex flex-col items-center justify-center flex-1 py-16 px-4 bg-gray-50/50 dark:bg-gray-800/50 rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-700 hover:border-orange-200 dark:hover:border-orange-900/50 transition-colors">
                <div className="bg-white dark:bg-gray-800 p-4 rounded-full shadow-sm mb-6 ring-1 ring-gray-100 dark:ring-gray-700">
                    <div className="bg-orange-50 dark:bg-orange-900/20 p-4 rounded-full">
                        <DollarSign className="h-12 w-12 text-orange-500" strokeWidth={1.5} />
                    </div>
                </div>
                <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">{t('payments.table.noPayments')}</h3>
      ...
      {/* Modals */}
      <PaymentDetailModal isOpen={isDetailModalOpen} onClose={closePaymentDetail} payment={selectedPayment} onEdit={handleEditFromDetail} refreshData={fetchPayments} />
      <Modal isOpen={isFormOpen} onClose={closePaymentForm} title={selectedPayment ? t('payments.modals.paymentForm.editTitle') : t('payments.modals.paymentForm.newTitle')} size="lg">
        <PaymentForm payment={selectedPayment} onSuccess={handleFormSuccess} onCancel={closePaymentForm} />
      </Modal>
      <Modal isOpen={confirmationModal.isOpen} onClose={() => setConfirmationModal(p => ({ ...p, isOpen: false }))} title={t('payments.modals.delete.title')} size="sm">
        <div className="p-6 text-center">
          ...
          <p className="mb-6 text-gray-600 dark:text-gray-300">{t('payments.modals.delete.description', { paymentName: confirmationModal.paymentName })}</p>
