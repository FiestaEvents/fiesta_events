  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const serverStats =
          dashboardData?.data?.stats || dashboardData?.stats || {};
        ...
        const occupancyRate =
          maxCapacity > 0
            ? Math.min(
                Math.round((currentMonthEventsCount / maxCapacity) * 100),
                100
              )
            : 0;
        ...
        <div>
          ...
          <p className="text-gray-500 mt-2">
            {t(
              ...
              "Here is what is happening at your venue today."
            )}
          </p>
        </div>
          ...
          {activity.riskyEvents.length > 0 && (
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-red-100 dark:border-red-900/30 p-6 relative overflow-hidden">
              ...
              <h3 className="font-bold text-gray-900 dark:text-white flex items-center gap-2 mb-4">
                ...
                {t(
                  "dashboard.alerts.missing_partners",
                  "Events Missing Partners"
                )}
              </h3>
...
const QuickActionBtn = ({ label, icon: Icon, onClick }) => (
  ...
  >
    <Icon
      size={20}
      className="mb-2 text-orange-500 group-hover:scale-110 transition-transform"
    />
