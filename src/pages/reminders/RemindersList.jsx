...
const RemindersList = () => {
  ...
  const fetchReminders = useCallback(async () => {
    try {
      ...
      let data = response?.data?.data?.reminders || response?.data?.reminders || response?.reminders || [];
  ...
  // Helpers
  const getPriorityColor = (p) => ({ urgent: "danger", high: "warning", medium: "info", low: "secondary" })[p] || "secondary";
  const getTypeColor = (tp) => ({ event: "primary", payment: "warning", task: "purple", maintenance: "orange", followup: "success" })[tp] || "secondary";
  const getStatusColor = (st) => ({ completed: "success", active: "info", snoozed: "warning", cancelled: "danger" })[st] || "secondary";
  ...
  const columns = [
    ...
    },
    {
      ...
      width: "25%",
      render: (row) => (
        <div className="text-gray-500 truncate max-w-xs" title={row.description}>
          {row.description || "-"}
        </div>
      ),
    },
    {
      ...
      render: (row) => (
        <div className="text-gray-600 dark:text-gray-400 flex items-center gap-2">
          ...
          {row.reminderTime && <span className="text-xs bg-gray-100 dark:bg-gray-700 px-1 rounded">{row.reminderTime}</span>}
        </div>
      ),
    },
    {
        ...
        return (
          <div className="flex justify-center gap-1">
            <Button
              ...
              onClick={(e) => {
                e.stopPropagation();
                isSnoozed ? handleUnsnooze(row) : setSnoozeOptionsModal({ isOpen: true, reminder: row });
              }}
    ...
    return (
      <div className="mt-6 border-t border-gray-200 dark:border-gray-700 pt-4 flex flex-col sm:flex-row justify-between items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
        ...
        <div className="flex items-center gap-2">
          {totalPages > 1 && <Pagination currentPage={page} totalPages={totalPages} onPageChange={setPage} pageSize={null} />}
          <div className="flex items-center gap-2">
            <span>Per page:</span>
            <select
              value={limit}
  ...
  return (
    <div className="space-y-6 p-6 bg-white dark:bg-[#1f2937] rounded-lg shadow-md min-h-[500px] flex flex-col">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 shrink-0">
        <div>
          ...
          <p className="mt-1 text-gray-500 dark:text-gray-400">
            {t("reminders.subtitle")} {hasInitialLoad && totalCount > 0 && ` • ${t("reminders.notifications.showingResults", { count: reminders.length, total: totalCount })}`}
          </p>
        </div>
      ...
      {hasInitialLoad && !showEmptyState && (
        <div className="p-4 bg-white dark:bg-gray-800 rounded-lg flex flex-col sm:flex-row gap-4 shrink-0">
          ...
          <div className="sm:w-40">
            <Select value={status} onChange={(e) => { setPage(1); setStatus(e.target.value); }} options={[
          ...
          <div className="sm:w-40">
            <Select value={priority} onChange={(e) => { setPage(1); setPriority(e.target.value); }} options={[
      ...
      <div className="flex-1 flex flex-col relative">
        ...
        {showData && (
          <>
            <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
              <Table columns={columns} data={reminders} loading={loading} onRowClick={(row) => { setSelectedReminder(row); setIsDetailModalOpen(true); }} striped hoverable />
            </div>
            {renderPagination()}
          </>
        )}
        ...
        {showNoResults && (
          <div className="flex flex-col items-center justify-center flex-1 py-12">
            ...
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">{t("reminders.noResults")}</h3>
            <p className="text-gray-500 dark:text-gray-400 text-center max-w-sm mb-6">{t("reminders.notifications.filtersCleared")}</p>
            <Button onClick={handleClearFilters} variant="outline" icon={X}>{t("reminders.clearFilters")}</Button>
          </div>
        )}
        ...
        {showEmptyState && (
          <div className="flex flex-col items-center justify-center flex-1 py-16 px-4 bg-gray-50/50 dark:bg-gray-800/50 rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-700 hover:border-orange-200 dark:hover:border-orange-900/50 transition-colors">
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">{t("reminders.noReminders")}</h3>
            <p className="text-gray-500 dark:text-gray-400 text-center max-w-sm mb-8 leading-relaxed">
              Set up reminders for important tasks, payments, and events so you never miss a deadline.
            </p>
            <Button onClick={() => { setSelectedReminder(null); setIsFormOpen(true); }} variant="primary" size="lg" icon={Plus} className="shadow-lg shadow-orange-500/20 hover:shadow-orange-500/30 transition-shadow">
      ...
      {/* Modals */}
      <ReminderDetailModal isOpen={isDetailModalOpen} onClose={() => setIsDetailModalOpen(false)} reminder={selectedReminder} onEdit={(r) => { setSelectedReminder(r); setIsDetailModalOpen(false); setIsFormOpen(true); }} refreshData={fetchReminders} />
      ...
      <Modal isOpen={confirmationModal.isOpen} onClose={() => setConfirmationModal((p) => ({ ...p, isOpen: false }))} title={t("reminders.modals.delete.title")} size="sm">
        <div className="p-6 text-center">
          ...
          <p className="mb-6 text-gray-600 dark:text-gray-300">{t("reminders.modals.delete.description", { name: confirmationModal.reminderName })}</p>
