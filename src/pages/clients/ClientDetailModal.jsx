...
const ClientDetailModal = ({
  ...
  const handleDelete = async () => {
    try {
      setIsDeleting(true);
      await promise(clientService.delete(client._id), {
      ...
      <Modal isOpen={isOpen} onClose={onClose} title={client.name} size="md">
        <div className="space-y-6">
          ...
          <div>
            ...
            <InfoRow
              icon={Mail}
              label={t("clientDetail.labels.email", "Email")}
              value={client.email}
              color="blue"
            />
            <InfoRow
              icon={Phone}
              label={t("clientDetail.labels.phone", "Phone")}
              value={client.phone}
              color="orange"
            />
            <InfoRow
              icon={Building}
              label={t("clientDetail.labels.company", "Company")}
            ...
            {client.address && (
              <InfoRow
                ...
                value={[client.address.street, client.address.city]
                  .filter(Boolean)
                  .join(", ")}
                color="blue"
              />
            ...
            <InfoRow
              icon={Calendar}
              label={t("clientDetail.labels.created")}
              ...
              {client.tags?.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-3">
                  {client.tags.map((tag, i) => (
                    <Badge key={i} variant="secondary" icon={<Tag size={10} />}>
                      {tag}
                    </Badge>
                  ))}
                </div>
