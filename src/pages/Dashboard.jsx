    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          {Icon && (
            <Icon className={`${iconColorClasses[iconColor]}`} size={20} />
          )}
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="mt-4 flex items-center justify-between">
            {trend !== undefined && (
              <div
                className={`flex items-center text-xs font-bold ${trend >= 0 ? "text-green-600" : "text-red-600"}`}
              >
                {trend >= 0 ? (
            ...
            {subValue && (
              <span
                className="text-xs text-gray-400 ml-auto truncate max-w-[120px]"
                title={subValue}
              >
                {subValue}
              </span>
...
const MiniCalendar = ({ events = [], displayDate }) => {
  ...
  const getDayStatus = (day) => {
    try {
      ...
      return ["confirmed", "completed", "paid", "in-progress"].includes(status)
        ? "booked"
        : "pending";
    } catch {
      return null;
      ...
      try {
        ...
        if (allEvents.length > 0) {
          ...
          const nextEvent = sortedEvents.find(
            (e) => new Date(e.startDate) >= new Date().setHours(0, 0, 0, 0)
          );
        ...
        if (invoiceStats.overdue > 0) {
          actionItems.push({
            type: "critical",
            text: t("dashboard.actions.overdue_invoices", {
              count: invoiceStats.overdue,
            }),
        ...
        if (pendingEvents.length > 0) {
          actionItems.push({
            type: "warning",
            text: t("dashboard.actions.new_enquiries", {
        ...
        if (urgentTasks.length > 0) {
          actionItems.push({
            type: "info",
            text: t("dashboard.actions.urgent_tasks", {
        ...
        // Pipeline
        const quotedEvents = allEvents.filter(
          (e) => e.status === "quoted" || e.quoted
        );
        ...
        const newClients = Object.values(clientEvents).filter(
          (count) => count === 1
        ).length;
        const returningClients = Object.values(clientEvents).filter(
          (count) => count > 1
        ).length;
        const avgLTV =
        ...
        const cancellationRate =
          allEvents.length > 0
            ...
            : 0;
        const cancelledRevenue = cancelledEvents.reduce(
          (sum, e) => sum + (e.pricing?.totalAmount || 0),
          0
        );
        ...
        const lastMonthAvgValue =
          lastMonthEvents.length > 0
            ? lastMonthEvents.reduce(
                (sum, e) => sum + (e.pricing?.totalAmount || 0),
                0
        ...
        allEvents.forEach((event) => {
          ...
          else if (["confirmed", "in-progress", "paid"].includes(status))
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900 p-4 md:p-6 lg:p-8 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  {t("dashboard.today.expected_revenue")}
                </p>
                <p className="text-2xl font-bold">
                  {formatCurrency(
                    activity.todayEvents.reduce(
                      (sum, e) => sum + (e.pricing?.totalAmount || 0),
                      0
                    )
                  )}
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  {t("dashboard.today.pending_tasks")}
                </p>
                <p className="text-2xl font-bold">
                  {metrics.operations.pendingTasks}
                </p>
              </div>
            </div>
          </div>
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <p
                  className={`text-2xl font-bold ${cashFlow.net >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                >
              ...
              <div className="flex items-center justify-between p-4 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800">
                <div>
                  ...
                  <p className="text-sm text-red-700 dark:text-red-400">
                    1 {t("dashboard.financial.invoices")} -{" "}
                    {t("dashboard.financial.critical")}
                  </p>
                </div>
          ...
          <div>
            ...
            <div className="mt-4 bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                {metrics.occupancy.totalEvents}/{30}{" "}
                {t("dashboard.calendar.days_booked")}
              </p>
            </div>
          </div>
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-500 dark:text-gray-400">
                            {event.clientId?.name ||
                              t("dashboard.calendar.unknown_client")}{" "}
                            • {event.guestCount || 0}{" "}
                            {t("dashboard.calendar.guests")}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                    ...
                    <p
                      className={`text-2xl font-bold ${metrics.revenue.growth >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                    >
            ...
            {activeTab === "events" && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  {Object.entries(insights.eventCategories).map(
                    ([type, count]) => (
                      <div
                        key={type}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {[
                      {
                        label: t("dashboard.performance.enquiries"),
                        value: insights.pipeline.enquiries,
                        color: "yellow",
                      },
                      {
                        label: t("dashboard.performance.quoted"),
                        value: insights.pipeline.quoted,
                        color: "blue",
                      },
                      {
                        label: t("dashboard.performance.confirmed"),
                        value: insights.pipeline.confirmed,
                        color: "green",
                      },
                      {
                        label: t("dashboard.performance.completed"),
                        value: insights.pipeline.completed,
                        color: "purple",
                      },
                    ].map((stage, idx) => (
                    ...
                    <div className="flex justify-between text-sm mb-1">
                      ...
                      <span className="font-bold text-gray-900 dark:text-white">
                        {insights.pipeline.enquiries > 0
                          ? Math.round(
                              (insights.pipeline.confirmed /
                                insights.pipeline.enquiries) *
                                100
                            )
                          : 0}
                        %
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          >
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-gray-600 dark:text-gray-400">
                  {t("dashboard.insights.cancellation_rate")}
                </span>
              ...
              <div className="flex justify-between items-center">
                <span className="text-gray-600 dark:text-gray-400">
                  {t("dashboard.insights.revenue_lost")}
                </span>
                <span className="text-lg font-semibold text-gray-700 dark:text-gray-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          ...
          <div className="space-y-4">
            {activity.pendingLeads.length > 0 ? (
              activity.pendingLeads.map((event) => (
                <div
                  key={event._id}
