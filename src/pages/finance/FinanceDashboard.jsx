...
const FinanceDashboard = () => {
  ...
  const [financialData, setFinancialData] = useState({
    ...
    upcomingPayments: [
      {
        id: 1,
        description: "Event Deposit - Johnson Wedding",
        amount: 2500,
        dueDate: "2025-11-08",
        type: "income",
      },
      {
        id: 2,
        description: "Partner Payment - Catering Co",
        amount: 1800,
        dueDate: "2025-11-10",
        type: "expense",
      },
      {
        id: 3,
        description: "Event Balance - Tech Corp Conference",
        amount: 4200,
        dueDate: "2025-11-12",
        type: "income",
      },
    ...
    recentTransactions: [
      {
        id: 1,
        description: "Birthday Party - Smith",
        type: "income",
        amount: 3500,
        date: "2025-11-01",
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
              ...
              <span className="text-lg font-bold text-blue-600 dark:text-blue-400">
                {formatCurrency(
                  financialData.eventFinancials.avgProfitPerEvent
                )}
              </span>
            </div>
            ...
            {[
              {
                ...
                amount: financialData.venueExpenses.staffSalary,
                color: "blue",
              },
              {
                label: "Utilities",
                amount: financialData.venueExpenses.utilities,
                color: "green",
              },
              {
                label: "Maintenance",
                amount: financialData.venueExpenses.maintenance,
                color: "yellow",
              },
              {
                label: "Marketing",
                amount: financialData.venueExpenses.marketing,
                color: "purple",
              },
              {
                label: "Insurance",
                amount: financialData.venueExpenses.insurance,
                color: "pink",
              },
              {
                label: "Other",
                amount: financialData.venueExpenses.other,
                color: "gray",
              },
            ].map((expense, index) => {
              const percentage =
                (expense.amount / financialData.venueExpenses.total) * 100;
              return (
                <div key={index}>
          ...
          <div className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-sm text-gray-600 dark:text-gray-400">
                    {formatCurrency(financialData.budgetTracking.monthly.spent)}{" "}
                    /{" "}
                    {formatCurrency(
                      financialData.budgetTracking.monthly.budget
                    )}
                  </span>
                  <span
              ...
              <div className="relative h-3 bg-gray-100 dark:bg-gray-900/50 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(financialData.budgetTracking.monthly.percentage, 100)}%`,
                  }}
                />
              </div>
              <p className="text-xs text-gray-500 dark:text-gray-500 mt-1">
                {formatCurrency(financialData.budgetTracking.monthly.remaining)}{" "}
                remaining
              </p>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-sm text-gray-600 dark:text-gray-400">
                    {formatCurrency(
                      financialData.budgetTracking.eventBudgets.spent
                    )}{" "}
                    /{" "}
                    {formatCurrency(
                      financialData.budgetTracking.eventBudgets.allocated
                    )}
                  </span>
              ...
              <div className="relative h-3 bg-gray-100 dark:bg-gray-900/50 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(financialData.budgetTracking.eventBudgets.percentage, 100)}%`,
                  }}
                />
              </div>
              <p className="text-xs text-gray-500 dark:text-gray-500 mt-1">
                {formatCurrency(
                  financialData.budgetTracking.eventBudgets.remaining
                )}{" "}
                available
              </p>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-sm text-gray-600 dark:text-gray-400">
                    {formatCurrency(
                      financialData.budgetTracking.venueBudgets.spent
                    )}{" "}
                    /{" "}
                    {formatCurrency(
                      financialData.budgetTracking.venueBudgets.allocated
                    )}
                  </span>
                  <span
                    className={`text-sm font-semibold ${
              ...
              <div className="relative h-3 bg-gray-100 dark:bg-gray-900/50 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(financialData.budgetTracking.venueBudgets.percentage, 100)}%`,
                  }}
                />
              </div>
              <p
              ...
              >
                {financialData.budgetTracking.venueBudgets.remaining < 0
                  ...
                  : ""}
                {formatCurrency(
                  Math.abs(financialData.budgetTracking.venueBudgets.remaining)
                )}
              </p>
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          ...
          <div className="space-y-3">
            {financialData.topPerformingEvents.map((event, index) => (
              <div
                key={event.id}
                className="p-4 bg-gradient-to-r from-blue-50 to-transparent dark:from-blue-900/20 dark:to-transparent rounded-lg"
              >
